...
class Blockchain {
  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (timeElapsed < seconds) {
        const isVerfiied = bitcoinMessage.verify(message, address, signature);
        if (!isVerfiied) {
          return reject("block not verification failed");
        }
  ...
  getBlockByHash(hash) {
    let self = this;
    return new Promise((resolve, reject) => {
      const filteredBlock = self.chain.filter(
  ...
  validateChain() {
    ...
    return new Promise(async (resolve, reject) => {
      self.chain.forEach(async (block, index) => {
        ...
        if (!isBlockValid) {
          return errorLog.push("block not valid");
        }
        if (
          block?.height !== 0 &&
          self.chain[index - 1]?.hash !== block?.previousBlockHash
        ) {
          return errorLog.push(`block with ${block?.height} is not valid`);
        }
      });
      resolve(errorLog);
    });
